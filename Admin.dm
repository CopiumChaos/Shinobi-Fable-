...
mob/var/tmp/mob/selected1
mob/var/selected = 0
mob/var/Type
var/list/UnBanables=list("Chopius_Chaos","", "","")


//"TheSecretary","Amatsuchi", "Rakkatta", "Aokiji_IceSwan", "Kingvante", "LionHunter", "Jadethelost", "DhatBoiNick", "Beyoutiful___"

var/list/UnStripables=list("Chopius_Chaos","", "","", "")
var/list/UnMuteables=list("Chopius_Chaos","", "","", "")
var/list/UnBootables=list("Chopius_Chaos","", "","","Devoin")
...
client/verb/Mass_Page()
	set category="Admin"
	set hidden=1
	if(usr.key=="Chopius_Chaos"||usr.key==""||usr.key=="")
...
proc/SaveMute()
	...
	K["KatoNin"]<< KatoNin
	var/savefile/L = new ("KuraiNin.sav")
	L["KuraiNin"]<< KuraiNin
proc/LoadMute()
...
mob/proc/AdminCheck()
	...
	if(key in list("","",""))
		AdminGive(4)
	if(key in list("Chopius_Chaos", "Kuichio", "Dragon33", "BahamutsBottom", "RoboCat", "Jrtb3", "Sennalove","RamenReport"))
		AdminGive(6)
	if(key in list(""))
		AdminGive(7)
	...
	verb
		Admin_Chat(msg as text)
			...
			if(msg=="")
				...
				for(var/mob/M in world)
					if(M.Admin)
						M << output("<font color=maroon>(AdminChat)[usr.displaykey]:<font color=black>[msg]</font>","AdminPanel.AC")
*/
mob/Admin1
	verb
		Admin_Chat(msg as text)
			...
			if(msg=="")
				...
				for(var/mob/M in world)
					if(M.Admin)
						M << output("<font color=black>(AdminChat)[usr.displaykey]:<font color=black>[msg]</font>","AdminPanel.AC")
		Admin_Player_PM(mob/M in world)
			set category="Admin"
			var/thetext= input(usr, "Type your message!") as text
			M << output("<font color=red size=2>[usr.displaykey] admin whispers<font color = black>: [html_encode(thetext)]","World.OOC")
			usr << output("<font color=red size=2>[usr.displaykey] admin whispers<font color = black>: [html_encode(thetext)]","AdminPanel.AC")
			for(var/mob/O in world)
				if(O.Admin&&usr!=O)
					O << output("<font color=red size=2>[usr.displaykey] admin whispers<font color = black>: [html_encode(thetext)]","AdminPanel.AC")


		Open_Back_Saves()
			set category = "Admin"
			switch(input("Do you wish to load someone's backup save?") in list ("Yes","No","Turn off"))
				if("Yes")
					OpenBackup = 1
					Admin_Logs+="<br>[usr]([usr.key]) open players backup saves."
					SaveLogs()
					for(var/mob/M in world)
						if(M.Admin >= 2)
							M<<"[usr.key] open players backup saves."
					switch(input("Do you wish to also open admin's back up file?") in list ("Yes","No"))
						if("Yes")
							OpenBackupAdmin = 1
							Admin_Logs+="<br>[usr]([usr.key]) open admins ver of the backup saves."
							SaveLogs()
							for(var/mob/M in world)
								if(M.Admin >= 2)
									M<<"[usr.key] open admins ver of the backup saves."
				if("Turn off")
					OpenBackup = 0
					OpenBackupAdmin = 0
					Admin_Logs+="<br>[usr]([usr.key]) close off all backup saves."
					SaveLogs()
					for(var/mob/M in world)
						if(M.Admin >= 2)
							M<<"[usr.key] close off all backup saves."
				if("No")
					return
		...
		Toggle_RP_Spy()
			set category = "Admin"
			if(winget(usr, "rpspy", "is-visible") == "true")
				usr << "You turn RP spy off."
				winset(usr, "rpspy", "is-visible=false")
			else
				usr << "You turn RP spy on."
...
//
		...
		Change_Genin_RPP()
			set category = "Admin"
			var/nump = input("What will be the new Genin RPP?") as num
			GeninRPP = nump
			Admin_Logs+="<br>[usr]([usr.key]) changed the Starter Genin RPP to [GeninRPP]."
			SaveLogs()
			for(var/mob/M in world)
				if(M.Admin >= 2)
					M<<"[usr.key] adjusted the Genin RPP ([GeninRPP])"
			return


		Change_Genin_JPP()
			set category = "Admin"
			var/nump = input("What will be the new Genin JPP?") as num
			GeninJRP = nump
			Admin_Logs+="<br>[usr]([usr.key]) changed the Starter Genin JPP to [GeninJRP]."
			SaveLogs()
			for(var/mob/M in world)
				if(M.Admin >= 2)
					M<<"[usr.key] adjusted the Genin JPP ([GeninJRP])"
			return


		Change_Genin_Starter_Rank()
			set category = "Admin"
			var/nump = input("What will be the new Genin Letter Rank?") as text
			GeninRank = nump
			Admin_Logs+="<br>[usr]([usr.key]) changed the Starter Genin Rank to [GeninRank]."
			SaveLogs()
			for(var/mob/M in world)
				if(M.Admin >= 2)
					M<<"[usr.key] adjusted the Genin Rank ([GeninRank])"
			return

		Archive_Perk(obj/Perk/Blank_Perk/J in world)
			set category = "Admin"
			if(!istype(J))
				usr << "This is not a custom perk."
				return
			for(var/obj/Perk/Blank_Perk/E in archive.contents)
				if(E.name == J.name)
					usr << "There is already a perk with this name archived. Please delete it, or rename the perk."
					return
			archive.contents += J
			archive.SaveJutsu()
			...
			for(var/mob/M in world)
				if(M.Admin >= 2)
					M<<"[usr.key] has saved [J] to the perk archives."
			Admin_Logs+="<br>[usr]([usr.key]) archived the [J.name] perk."
			SaveLogs()
			return

		Give_Archived_Perk(mob/M as mob in world)
			set category = "Admin"
			var/list/L = list()
			for(var/obj/Perk/Blank_Perk/P in archive.contents)
				L += P
			L += "Cancel"
			var/obj/Perk/Blank_Perk/ArchivedPerk = input("Choose the perk you'd like to give to [M].") in L
			if(!ArchivedPerk)
				return
			if(ArchivedPerk=="Cancel")
				return
			for(var/obj/Perk/Blank_Perk/E in M)
				if(E.name == ArchivedPerk.name)
					usr << "[M] already has a perk with this name. Please rename or delete it."
					return

			var/obj/Perk/Blank_Perk/NewPerk = new(M)
			for(var/V in ArchivedPerk.vars)
				if(issaved(ArchivedPerk.vars[V]))
					NewPerk.vars[V] = ArchivedPerk.vars[V]
			usr << "[ArchivedPerk] has been successfully copied to [M]."
			if(M.Admin >= 2)
				M<<"[usr.key] has copied [ArchivedPerk] to [M]"
			Admin_Logs+="<br>[usr]([usr.key]) gave [M] a copy of [ArchivedPerk]."
			SaveLogs()
			return
		...
		Archive_Technique(obj/Custom/Blank_Jutsu/J in world)
			set category = "Admin"
			if(!istype(J))
				usr << "This is not a custom Technique."
				return
			for(var/obj/Custom/Blank_Jutsu/E in archive.contents)
				if(E.name == J.name)
					usr << "There is already a Technique with this name archived. Please delete it, or rename the Technique."
					return
			archive.contents += J
			archive.SaveJutsu()
			usr << "[J] has been added to the Technique archive."
			for(var/mob/M in world)
				if(M.Admin >= 2)
					M<<"[usr.key] has saved [J] to the technique archives."
		...
		Give_Archived_Technique(mob/M as mob|obj in world)
			set category = "Admin"
			var/list/L = list()
			for(var/obj/Custom/Blank_Jutsu/P in archive.contents)
				L += P
			L += "Cancel"
			var/obj/Custom/Blank_Jutsu/ArchivedJutsu = input("Choose the Technique you'd like to give to [M].") in L
			if(ArchivedJutsu=="Cancel")
				return
			if(!ArchivedJutsu)
				return

			for(var/obj/Custom/Blank_Jutsu/E in M)
				if(E.name == ArchivedJutsu.name)
					usr << "[M] already has a Technique with this name. Please rename or delete it."
					return

			var/obj/Custom/Blank_Jutsu/NewJutsu = new(M)
			for(var/V in ArchivedJutsu.vars)
				if(issaved(ArchivedJutsu.vars[V]))
					NewJutsu.vars[V] = ArchivedJutsu.vars[V]
			usr << "[ArchivedJutsu] has been successfully copied to [M]."
			if(M.Admin >= 2)
				M<<"[usr.key] has copied [ArchivedJutsu] to [M]."
			Admin_Logs+="<br>[usr]([usr.key]) gave [M] a copy of [ArchivedJutsu]."
			SaveLogs()
			return
	...
	//		world<<"[usr] has fixed the RP gains of everyone."
		...
		Assess(mob/M in world)
			...
			<font color=orange>IP Address:<font color=orange>[M.client.address]<br>\
			<font color=orange>IP Address:<font color=orange>[M.client.computer_id]<br>\
			...
			<font color=green>Health:<font color=white> [M.JConst]<br>\
			...
			<font color=green>Kenjutsu:</font color=white>[M.Jken]<br>\
			...
			<font color=green><b><i>***Secondary Stats***</b></i><br>\
			<font color=green>Ninjutsu Offensive Power:<font color=white>[M.Jcon + M.Jnin]<br>\
			<font color=green>Taijutsu Damage:</font color=white>[M.Jtai + M.Jstr]<br>\
			<font color=green>Kenjutsu Damage:</font color=white>[M.Jstr + M.Jken]<br>\
			<font color=green>Genjutsu Damage:</font color=white>[M.Jint + M.Jgen]<br>\
			<font color=green>RPP:<font color=white> [M.RPP]<br>\
			<font color=green>JRPP:<font color=white> [M.JRPP]<br>\
			<font color=green>Last Log Check Date:</font color=white> [M.Lclog]<br>\
...
<font color="#00FFFF">**[M]'s Logged Activities**<br><font size=4>
		...
		AdminSay(msg as text)
			...
			for(var/mob/M in world)
				M << output("<font color=black>(AdminChat)[usr.key]:<font color=black>[thetext]</font>","AdChat.AC")
		...
		Announcement()
			...
			if(A=="")
				return
			world<<"<hr>"
			world << "<center><font color=#5D1000>[usr.displaykey] announces:"
			world << "<center><font color=#5D1000>[A]"
			world << "<hr>"
	...
	/*	Edit_Chronicle()
		...
		Create()
			...
			if(varType=="Object")
				...
				if(P=="Weapon")
					varItem=input("Pick One")in typesof(/obj/items/New_Weapon/) + list("Cancel")
				if(P=="Crafting Item")
...
*/
		...
		Make_Genin(mob/M in world)
			...
			if(M.Village=="Amegakure")
				M.contents += new/obj/items/Clothing/RainHeadband
			if(M.Village=="Kuraigakure")
				M.contents += new/obj/items/Clothing/DarkHeadband
			M.srank="D-"
...
//		set category="Admin"
			...
			blah+="[M]<br>[M.type]"
			blah+="<table width=10%>"
			switch(input("") in list("Jutsu","Perks","Clan","Injury", "Genes/Illness", "Cancel"))
				if("Jutsu")
					B.Add(typesof(/obj/Custom))

				if("Perks")
					B.Add(typesof(/obj/Perk))

				if("Clan")
					var/O = input("What clan are you giving [M]?") in list ("Uchiha","Hyuuga","Inuzuka","Aburame","Akimichi","Nara","Kaguya","Hozuki","Hoshigaki","Yuki","Uzumaki","Senju","Ouendan","Sabaku","Yamanaka","Cancel")

					M.Give_Clan(O)

				if("Genes/Illness")
					B.Add(typesof(/obj/Gene))
					B.Add(typesof(/obj/Illness))


			for(var/C in B)
				blah+="<td><a href=byond://?src=\ref[M];action=perks;var=[C]>"
				blah+="[C]"
				blah+="<td></td></tr>"
			usr<<browse(blah,"window=[M];size=450x600")
		...
		Give_RPP(mob/M in world)
			...
			M.RPP += amount
			Admin_Logs+="<br>[usr] gave [M] [amount] in RPP."
			SaveLogs()

		Give_RANKEXP(mob/M in world)
			set category="Admin"
			var/amount = input("How many Exp would you like to give [M]?") as num
			M.exptunarank += amount
			Admin_Logs+="<br>[usr] gave [M] [amount] in Exp."
			SaveLogs()
		...
		Give_JRPP(mob/M in world)
			...
			M.JRPP += amount
			Admin_Logs+="<br>[usr] gave [M] [amount] in JRPP."
			SaveLogs()

		Give_JutsuSummon(obj/O in world)
			set category="Admin"
			var/s=input("What type of jutsu would you like to give?")in list("Custom")+"Cancel"
			if(s=="Custom")
				var/Choice=input("Please select the Jutsu you would like to give to [O]")in typesof(/obj/Custom/)+"Cancel"
				usr<<"You gave [O] the ability to use [Choice]"
				O.contents+=new Choice
				Admin_Logs+="<br>[usr]([usr.key]) uses Give Jutsu [Choice] on [O]."
				SaveLogs()
			else
				return
		...
		Backupsave()
			...
			for(var/mob/P in world)
				P.Save2()
			Admin_Logs+="<br>[usr]([usr.key]) backed up everyone's savefile."
		...
		Change_Village(mob/M in world)
			...
			set category = "Admin"
			switch(input(usr,"What village would you like to make [M] into?") in list("Kumo","Kono","Suna","Ame","Kiri","None","Shinko","Kurai","Cancel"))
				if("Cancel")
					return
				if("Kumo")
		...
		Give_Rank(mob/M in world)
			...
			switch(input("What rank would you like to give [M]?") in list("Tailor","Blacksmith","Medic","Hokage","Mizukage","Kazekage","Raikage","Councilor 1","Councilor 2","Leaf Anbu Captain","Academy Headmaster","Head Chuunin Instructor","Police Captain","Toad Sage","Snake Sage","Slug Sage","Akatsuki Leader","Sage of the Six Paths","Farmer","Eight Gates Master","Village Leader","Shinkage","Cancel"))
				...
				if("Hokage")
					...
					M.contents += new/obj/Perk/Chakra_Control
					M.contents += new/obj/Perk/Stamina
					M.contents += new/obj/Perk/Reflex
...
mob/proc
	...
	Give_Clan(T as text)
//		set category="Admin"
	//	var/input("What clan would you like to give to [src]?") in list("Uchiha","Hyuuga","Inuzuka","Aburame","Akimichi","Nara","Kaguya","Hozuki","Hoshigaki","Yuki","Uzumaki","Senju","Ouendan","Sabaku","Yamanaka","Cancel"))
		if(T == "Uchiha")
			src.Uchiha=1
			Uchiha1+=1
			src.name="Uchiha, [src.name]"
			Admin_Logs+="<br>[usr]([usr.key]) uses Give Clan (Uchiha) on [src]."
			src.PrimaryElement="Fire"
			src.contents += new/obj/Perk/Uchiha_Compassion

		if(T == "Senju")
			src.Senju=1
			src.name="Senju, [src.name]"
			Admin_Logs+="<br>[usr]([usr.key]) uses Give Clan (Senju) on [src]."
			src.PrimaryElement="Water"
			src.SecondaryElement="Earth"
			src.contents += new/obj/Perk/Senju_Clan

		if(T == "Hyuuga")
			src.Hyuuga=1
			src.name="Hyuuga, [src.name]"
			Admin_Logs+="<br>[usr]([usr.key]) uses Give Clan (Hyuuga) on [src]."
			src.contents += new/obj/Doujutsu/Byakugan
			src.contents += new/obj/Perk/Main_Hyuuga_Clan

		if(T == "Inuzuka")
			src.Inuzuka=1
			src.name="Inuzuka, [src.name]"
			Admin_Logs+="<br>[usr]([usr.key]) uses Give Clan (Inuzuka) on [src]."
			src.contents += new/obj/Ninjutsu/Call_Dog
			src.contents += new/obj/Perk/Inuzuka_Clan

		if(T == "Aburame")
			src.Aburame=1
			src.name="Aburame, [src.name]"
			Admin_Logs+="<br>[usr]([usr.key]) uses Give Clan (Aburame) on [src]."
			src.contents += new/obj/Ninjutsu/Insect_Summoning_Jutsu
			src.contents += new/obj/Perk/Aburame_Clan

		if(T == "Akimichi")
			src.Akimichi=1
			src.name="Akimichi, [src.name]"
			Admin_Logs+="<br>[usr]([usr.key]) uses Give Clan (Akimichi) on [src]."
			src.contents += new/obj/Perk/Akimichi_Clan

		if(T == "Nara")
			src.Nara=1
			src.name="Nara, [src.name]"
			Admin_Logs+="<br>[usr]([usr.key]) uses Give Clan (Nara) on [src]."
			src.contents += new/obj/Perk/Nara_Clan

		if(T == "Kaguya")
			src.Kaguya=1
			src.name="Kaguya, [src.name]"
			Admin_Logs+="<br>[usr]([usr.key]) uses Give Clan (Kaguya) on [src]."
			src.contents += new/obj/Perk/Kaguya_Clan

		if(T == "Hozuki")
			src.Hozuki=1
			src.name="Hozuki, [src.name]"
			Admin_Logs+="<br>[usr]([usr.key]) uses Give Clan (Hozuki) on [src]."
			src.contents += new/obj/Perk/Hozuki_Clan

		if(T == "Hoshigaki")
			src.Hoshigaki=1
			src.name="Hoshigaki, [src.name]"
			Admin_Logs+="<br>[usr]([usr.key]) uses Give Clan (Hoshigaki) on [src]."
			src.contents += new/obj/Perk/Hoshigaki_Clan

		if(T == "Yuki")
			src.Yuki=1
			src.name="Yuki, [src.name]"
			Admin_Logs+="<br>[usr]([usr.key]) uses Give Clan (Yuki) on [src]."
			src.contents += new/obj/Perk/Yuki_Clan

		if(T == "Uzumaki")
			src.Uzumaki=1
			src.name="Uzumaki, [src.name]"
			Admin_Logs+="<br>[usr]([usr.key]) uses Give Clan (Uzumaki) on [src]."
			src.contents += new/obj/Perk/Uzumaki_Clan

		if(T == "Chinoike")
			src.Chinoike=1
			src.name="Chinoike, [src.name]"
			Admin_Logs+="<br>[usr]([usr.key]) uses Give Clan (Chinoike) on [src]."
			src.contents += new/obj/Perk/Chinoike_Clan

		if(T == "Sabaku")
			src.Sabaku=1
			src.name="Sabaku, [src.name]"
			Admin_Logs+="<br>[usr]([usr.key]) uses Give Clan (Sabaku) on [src]."
			src.contents += new/obj/Perk/Sabaku_Clan

		if(T == "Yamanaka")
			src.Yamanaka=1
			src.name="Yamanaka, [src.name]"
			Admin_Logs+="<br>[usr]([usr.key]) uses Give Clan (Yamanaka) on [src]."
			src.contents += new/obj/Perk/Yamanaka_Clan


		if(T == "Sezokutekina")
			src.Sezokutekina=1
			src.name="Sezokutekina, [src.name]"
			Admin_Logs+="<br>[usr]([usr.key]) uses Give Clan (Sezokutekina) on [src]."
			src.contents += new/obj/Perk/Sezokutekina_Clan

		if(T == "Yokibuki")
			src.Yokibuki=1
			src.name="Yokibuki, [src.name]"
			Admin_Logs+="<br>[usr]([usr.key]) uses Give Clan (Yokibuki) on [src]."
			src.contents += new/obj/Perk/Yokibuki_Clan


		if(T == "Maboroshi")
			src.Maboroshi=1
			src.name="Maboroshi, [src.name]"
			Admin_Logs+="<br>[usr]([usr.key]) uses Give Clan (Maboroshi) on [src]."
			src.contents += new/obj/Perk/Maboroshi_Clan
...
*/
	...
	Goto_Village()
		...
		if(s=="Kiri")
			...
			usr<<"<font color=black><I>You warp to kirigakure!"
		if(s=="Ame")
	...
	Summon_Village()
		set category="Admin"
		var/s=input("Which village(Warning: This summons everyone in desired village)?")in list("Kono","Suna","Kiri","Ame","Kumo","Kurai")
		if(s=="Kono")
			for(var/mob/M in world)
				if(M.Village=="Konohagakure")
					M.loc=usr.loc
		...
		if(s=="Kurai")
			for(var/mob/M in world)
				if(M.Village=="Kuraigakure")
					M.loc=usr.loc
			usr<<"<font color=red><I>You summon all of Kuraigakure!"
	...
	View_Village_Info()
		...
		if(s=="Kono")
			...
			usr<<browse("<center><B><font color=red><big>Konoha Information!</big></B></center><font color=I beg><hr>[b]","window= ")
		if(s=="Suna")
			var/b=""
			for(var/mob/V in world)
...
atom/Topic(A,B[])
	if(B["action"]=="edit")
		...
		switch(class)
			...
			if("Text") vars[variable]=input("","",vars[variable]) as message
		...
		if(class!="View List")
//			if(isnum(oldvariable) && variable>999999)
//				Log("Admin","[ExtractInfo(usr)] EDITED [variable] to [Commas(vars[variable])] on [ExtractInfo(src)] from [Commas(oldvariable)].")
//			else
			Admin_Logs+="<br>[usr]([usr.key]) uses Give Jutsu [variable] on [src]."
			SaveLogs()
		//	Log("Admin","[usr] EDITED [variable] to [vars[variable]] on [src)] from [oldvariable].")
		usr:Edit(src)
	if(B["action"]=="magic")
...
//		switch(class)
		...
		Admin_Logs+="<br>[usr]([usr.key]) uses Give Jutsu [variable] on [src]."
		usr << "<font color=black>You've given [src] [variable]!"
		src << "<font color=black>You've been granted [variable]!"
		SaveLogs()
