...
obj/Ninjutsu/Insect_Summoning_Jutsu
	icon='Skillcard.dmi'
	icon_state="Bugs"
	Click()
		if(src in usr.contents)
			for(var/mob/Bugs/Bugs/R in world)
				if(R.Owner==usr.key)
					usr << "You may only summon one swarm of Bugs at a time."
					return
...
obj/Ninjutsu/Parasitic_Insects_Jutsu
	var/skill=1
	icon='Skillcard.dmi'
		...
		if(src in usr.contents)
			if(usr.move)
				...
				if(prob(10/skill/skill/skill))
					usr<<"Your understanding of the Kikaichuu no Jutsu has increased."
					skill+=1
				usr.firing=1
				src.Active=1
				...
				viewers(16) << "<font color=[usr.SayFont]>[usr] yells<font color = white>: Parasitic Insects Jutsu!"
...
obj/Ninjutsu/Hidden_Jutsu:_Beetle_Sphere
	icon='Skillcard.dmi'
		...
		if(src in usr.contents)
			if(usr.move)
				...
				for(var/mob/M in oview())
					V.Add(M)
				var/mob/A=input("Who would you like to use the Hidden Jutsu: Beetle Sphere on?") in V
				if(A==null)
					return
				if(usr.Ninjutsu >= A.Ninjutsu*1.1)
					...
					A.Bug()
					A.icon_state="KO"
					viewers(16) << "<font color=[usr.SayFont]>[usr] yells<font color = white>: Hidden Jutsu: Beetle Sphere!"
