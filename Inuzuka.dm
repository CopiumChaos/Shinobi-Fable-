	...
	Click()
		if(src in usr.contents)
			...
			for(var/mob/Dog/D in world)
				if(D.Owner==usr.key)
					usr << "You may only summon one Dog at a time."
					return
			usr.Chakra-=50
			var/mob/Dog/D = new()
			D.Owner = usr.key
	...
	Click()
		if(src in usr.contents)
			...
			if(locate(/obj/items/Weapon/Enma_Pole,usr.contents))
				usr << "You have already summoned King Enma."
				return
	...
	Click()
		if(src in usr.contents)
			...
			for(var/mob/Slug/D in world)
				if(D.Owner==usr.key)
					usr << "You may only summon one Slug at a time."
					return
			usr.Chakra-=50
			var/mob/Slug/D = new()
			D.Owner = usr.key
	...
	Click()
		if(src in usr.contents)
			for(var/mob/Big_Snake/Big_Snake/E in world)
				if(E.Owner==usr.key)
					usr << "You may only summon one Snake at a time."
					return
			if(usr.Chakra<usr.MaxChakra*(0.80/Kuchiyoseskill))
				return
			if(!usr.move)
	...
	Click()
		if(src in usr.contents)
			...
			for(var/mob/Wolf/W in world)
				if(W.Owner==usr.key)
					usr << "You may only summon one Wolf at a time."
					return
			usr.Chakra-=70
			var/mob/Wolf/W = new()
			W.Owner = usr.key
	...
	Click()
		if(src in usr.contents)
			...
			for(var/mob/Cat/C in world)
				if(C.Owner==usr.key)
					usr << "You may only summon one Cat at a time."
					return
			usr.Chakra-=50
			var/mob/Cat/C = new()
			C.Owner = usr.key
...
mob/var/Gatsuga=0
obj/Ninjutsu/ManBeast_Ultimate_Taijutsu:_Fang_over_Fang
	var/skill=1
	var/Gatsuga=0
	icon='Skillcard.dmi'
	icon_state="Gatsuga"
		...
		if(src in usr.contents)
			if(usr.move)
				...
				if(prob(10/skill/skill/skill))
					usr<<"Your understanding of the Man-Beast Ultimate Taijutsu: Fang over Fang has increased."
					skill+=1
				src.Active=1
				...
				viewers(16) << "<font color=[usr.SayFont]>[usr] yells<font color = white>: Man-Beast Ultimate Taijutsu: Fang over Fang!"
