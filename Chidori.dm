		...
		if(src in usr.contents)
			if(!usr.Chidori)
				...
				if(prob(10/skill/skill/skill))
					usr<<"Your understanding of the Raiton: Chidori has increased."
					skill+=1
		...
		if(src in usr.contents)
			if(usr.move)
				...
				if(usr.Chakra<usr.MaxChakra*(0.45/skill))
					return
				if(prob(10/skill/skill/skill))
					usr<<"Your understanding of the Chidori Senbon no Jutsu has increased."
					skill+=1
				usr.firing=1
				src.Active=1
				...
				viewers(16) << "<font color=[usr.SayFont]>[usr] yells<font color = white>: Chidori Senbon no Jutsu!"
