...
//	obj
  // Click(location, control, params)
    //  if(location == "Weapons")
		//if(src in usr.contents)
				viewers(16) << output("<font color=grey>[usr] just activated [src.name]: <font color=purple>[src.description]","World.OOC")
...
///					src.suffix=""
	...
	Zanbatou
		icon='Zanbatou.dmi'
		icon_state="Inv"
		description="This is a normal iron zanbatou, within the right hands it becomes a capable entry level weapon +5 damage<br>(Hit points:25)<br>(Durability:15)"
...
////					usr.contents += new/obj/Ninjutsu/Renkuudan
	...
	Spear
		icon='Spear.dmi'
		icon_state="Inv"
		description="<br><font color=yellow>This is a normal wooden spear with an iron tip, a useful weapon within the right hands.+5 damage<br>(Hit points:25)<br>(Durability:15)"
		Click()
			if(src in usr.contents)
				if(!src.worn)
					src.worn = 1
	...
	Shuusui
		...
		icon_state="Inv"
		description="This is a normal weapon"
		Click()
			if(src in usr.contents)
				if(!src.worn)
					src.worn = 1
