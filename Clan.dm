...
<font color=green><font size=3>"}
var/KuraigakureNotes={"html><head><title>Kuraigakure Village Notes</title><body><body bgcolor="#000000">
<font color=green><font size=3>"}
	...
	verb
		View_Village_Page()
			...
			if(usr.Village=="Kuraigakure")
				usr<<browse("[KuraigakureNotes]","window=Village;size=300x500")
...
obj/EditVillagePage
	verb
		...
		Edit_Page()
			...
			if(usr.Village=="Kuraigakure")
				KuraigakureNotes=input("Edit the Village Notes!","Kuraigakure","[KuraigakureNotes]")as message
				var/savefile/F = new ("SSO/KuraigakureNotes.sav")
				F["Notes"]<< KuraigakureNotes
...
proc/LoadVillageNotes()
	...
	if(fexists("SSO/KumogakureNotes.sav"))
		var/savefile/F = new("SSO/KumogakureNotes.sav")
		F["Notes"]>> KumogakureNotes
	if(fexists("SSO/KuraigakureNotes.sav"))
		var/savefile/F = new("SSO/KuraigakureNotes.sav")
		F["Notes"]>> KuraigakureNotes
