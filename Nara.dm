...
obj/Ninjutsu/Shadow_Possession_Jutsu
	var/skill=1
	icon='Skillcard.dmi'
		...
		if(src in usr.contents)
			if(usr.move)
				...
				if(prob(10/skill/skill/skill))
					usr<<"Your understanding of Shadow Possession Jutsu has increased."
					skill+=1
				usr.firing=1
				src.Active=1
...
obj/Ninjutsu/Shadow_Strangle_Jutsu
	icon='Skillcard.dmi'
	icon_state="Neckbind"
	Click()
		if(src in usr.contents)
			if(usr.move)
				...
				for(var/mob/M in oview())
					V.Add(M)
				var/mob/A=input("Who would you like to use Shadow Strangle Jutsu on?") in V
				if(A==null)
					return
				if(A.move)
					return
				...
				if(usr.Ninjutsu >= A.Resistance*1.1)
					...
					A.icon_state="Focus"
					viewers(16) << "<font color=[usr.SayFont]>[usr] yells<font color = white>: Shadow Strangle Jutsu!"
					...
					usr << "[A] resists your Shadow Strangle Jutsu!"